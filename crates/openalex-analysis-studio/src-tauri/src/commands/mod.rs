...
#[derive(Default)]
pub(crate) struct VaultConnectionState {
    vault_path: Option<PathBuf>,
    latest_connection_generation: u64,
}

#[derive(Clone, Debug, Eq, Hash, PartialEq)]
pub(crate) struct SearchIndexTaskOwner {
    connection_generation: u64,
    vault_path: PathBuf,
}

pub(crate) struct SearchIndexTaskStore {
...
#[derive(Default)]
pub(crate) struct SearchIndexTaskRegistry {
...
pub(crate) struct ImportSourcesRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct ImportLiteratureRequest {
    preview: cistella_core::import::conflict::ImportPreview,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct SearchSourcesRequest {
    text: Option<String>,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct LocalSearchRequest {
    text: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct SearchIndexTaskControlRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct VaultRequestContext {
    expected_generation: u64,
    expected_vault_path: String,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct AppDirectoriesDto {
    config_dir: String,
...
pub struct HeadlessDoctorReport {
    ...
    pub recent_vaults_path: String,
}

pub fn headless_doctor_report(version: &'static str) -> CommandResult<HeadlessDoctorReport> {
    let executable = std::env::current_exe().map_err(|e| e.to_string())?;
    let executable_dir = executable
        .parent()
        .ok_or_else(|| "failed to resolve executable directory".to_string())?;
    let directories = AppDirectories::from_exe_dir(executable_dir).map_err(|e| e.to_string())?;
    Ok(HeadlessDoctorReport {
        version,
        executable: executable.display().to_string(),
        executable_dir: executable_dir.display().to_string(),
        mode: if directories.is_portable_mode() {
            "portable"
        } else {
            "installed"
        },
        portable_root: directories
            .portable_root()
            .map(|path| path.display().to_string()),
        config_dir: directories.config_dir().display().to_string(),
        cache_dir: directories.cache_dir().display().to_string(),
        recent_vaults_path: directories.recent_vaults_path().display().to_string(),
    })
}
...
pub(crate) enum SearchIndexTaskKind {
    Synchronize,
    Rebuild,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct LiteratureItemRequest {
    title: String,
    authors: Vec<String>,
...
#[tauri::command]
pub(crate) fn list_literature_items(state: tauri::State<AppState>) -> CommandResult<Value> {
    serde_json::to_value(
...
#[tauri::command]
pub(crate) fn create_literature_item(
...
#[tauri::command]
pub(crate) fn update_literature_item(
    item_id: String,
    req: LiteratureItemRequest,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn delete_literature_item(
    item_id: String,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
    let id = uuid::Uuid::parse_str(&item_id).map_err(|e| e.to_string())?;
...
#[tauri::command]
pub(crate) fn set_literature_item_favorite(
    item_id: String,
    favorite: bool,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn set_literature_item_tags(
    item_id: String,
    tags: Vec<String>,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn set_literature_item_reading_status(
    item_id: String,
    reading_status: String,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn list_document_assets(state: tauri::State<AppState>) -> CommandResult<Value> {
    let vault = vault_from_state(&state)?;
    let entries = vault
...
#[tauri::command]
pub(crate) fn import_document_asset(
    item_id: String,
    source_path: String,
    asset_kind: String,
    state: tauri::State<AppState>,
...
#[tauri::command]
pub(crate) fn link_external_document_asset(
    item_id: String,
    source_path: String,
    asset_kind: String,
    state: tauri::State<AppState>,
...
#[tauri::command]
pub(crate) fn set_document_asset_kind(
    item_id: String,
    asset_id: String,
    asset_kind: String,
    state: tauri::State<AppState>,
...
#[tauri::command]
pub(crate) fn set_document_asset_default(
    item_id: String,
    asset_id: String,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn remove_document_asset(
    item_id: String,
    asset_id: String,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn migrate_external_document_asset(
    item_id: String,
    asset_id: String,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn open_document_asset(
    item_id: String,
    asset_id: String,
    state: tauri::State<AppState>,
    app: tauri::AppHandle,
...
#[tauri::command]
pub(crate) fn start_reading_session(
    item_id: String,
    asset_id: String,
    state: tauri::State<AppState>,
    app: tauri::AppHandle,
...
#[tauri::command]
pub(crate) fn resume_reading_session(
    item_id: String,
    asset_id: String,
    state: tauri::State<AppState>,
    app: tauri::AppHandle,
...
#[tauri::command]
pub(crate) fn pause_reading_session(
    item_id: String,
    asset_id: String,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn end_reading_session(
    item_id: String,
    asset_id: String,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn list_recent_reading_sessions(state: tauri::State<AppState>) -> CommandResult<Value> {
    serde_json::to_value(
        vault_from_state(&state)?
...
#[tauri::command]
pub(crate) fn continue_reading_target(state: tauri::State<AppState>) -> CommandResult<Value> {
    serde_json::to_value(
        vault_from_state(&state)?
...
#[tauri::command]
pub(crate) fn continue_reading_session(
    state: tauri::State<AppState>,
    app: tauri::AppHandle,
) -> CommandResult<Value> {
    let vault = vault_from_state(&state)?;
...
#[tauri::command]
pub(crate) fn add_literature_vault_file(
    item_id: String,
    source_path: String,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn add_literature_external_file(
    item_id: String,
...
#[tauri::command]
pub(crate) fn remove_literature_file(
    item_id: String,
    file_id: String,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn set_literature_default_file(
    item_id: String,
    file_id: String,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn open_literature_file(
    item_id: String,
    file_id: String,
    state: tauri::State<AppState>,
    app: tauri::AppHandle,
...
#[tauri::command]
pub(crate) fn inspect_literature_import(
    format: ImportFormat,
    bytes: Vec<u8>,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn import_literature_file(
    format: ImportFormat,
...
#[tauri::command]
pub(crate) fn preview_openalex_work(
    record: SourceRecord,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
    let vault = vault_from_state(&state)?;
...
#[tauri::command]
pub(crate) async fn resolve_remote_metadata(
    identifier: String,
    force_refresh: bool,
...
#[tauri::command]
pub(crate) async fn import_by_identifier(
    identifier: String,
    strategy: String,
    state: tauri::State<'_, AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn clear_remote_metadata_cache(vault_path: String) -> CommandResult<()> {
...
#[tauri::command]
pub(crate) fn resolve_doi_local(
    raw_sources_dir: String,
    doi: String,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn list_local_openalex_works(
    raw_sources_dir: String,
    query: String,
    limit: usize,
    state: tauri::State<AppState>,
...
#[tauri::command]
pub(crate) fn local_search_index_state(state: tauri::State<AppState>) -> CommandResult<Value> {
    serde_json::to_value(vault_from_state(&state)?.search_index_state()).map_err(|e| e.to_string())
}

#[tauri::command]
pub(crate) fn local_search_index_issues(state: tauri::State<AppState>) -> CommandResult<Value> {
    let vault = vault_from_state(&state)?;
...
#[tauri::command]
pub(crate) fn local_search_task_state(state: tauri::State<AppState>) -> CommandResult<Value> {
    serde_json::to_value(visible_search_task_state(&state)?).map_err(|e| e.to_string())
}

#[tauri::command]
pub(crate) fn synchronize_local_search_index(
...
#[tauri::command]
pub(crate) fn rebuild_local_search_index(
    req: SearchIndexTaskControlRequest,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
    serde_json::to_value(start_search_index_task(
...
#[tauri::command]
pub(crate) fn cancel_local_search_index_task(
    req: SearchIndexTaskControlRequest,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn get_note(note_id: String, state: tauri::State<AppState>) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn annotation_resolution(
    annotation_id: String,
    req: VaultRequestContext,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn open_annotation_asset(
    annotation_id: String,
    req: VaultRequestContext,
    state: tauri::State<AppState>,
    app: tauri::AppHandle,
...
#[tauri::command]
pub(crate) fn inspect_sources(raw_sources_dir: String) -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn vault_context(state: tauri::State<AppState>) -> CommandResult<Value> {
    let path = state
        .connection
        .lock()
        .map_err(|e| e.to_string())?
...
#[tauri::command]
pub(crate) fn source_adapters() -> CommandResult<Value> {
    serde_json::to_value(available_source_adapters()).map_err(|e| e.to_string())
}

#[tauri::command]
pub(crate) fn app_directories() -> CommandResult<AppDirectoriesDto> {
    let dirs = current_app_directories()?;
    app_directories_to_dto(&dirs)
}

#[tauri::command]
pub(crate) fn is_portable_mode() -> CommandResult<bool> {
...
#[tauri::command]
pub(crate) fn get_app_version() -> CommandResult<String> {
    cistella_core::read_version_from_tauri_conf(&tauri_conf_path()).map_err(|e| e.to_string())
}

#[tauri::command]
pub(crate) fn check_update() -> CommandResult<Value> {
...
#[tauri::command]
pub(crate) fn recent_vaults() -> CommandResult<Vec<RecentVaultDto>> {
    let dirs = current_app_directories()?;
...
#[tauri::command]
pub(crate) fn migrate_vaults_to_portable(
    vaults: Vec<RecentVaultDto>,
) -> CommandResult<Vec<RecentVaultDto>> {
    let dirs = current_app_directories()?;
    let vaults: Vec<RecentVault> = vaults.into_iter().map(recent_vault_from_dto).collect();
...
#[tauri::command]
pub(crate) fn migrate_vaults_to_installed(
    vaults: Vec<RecentVaultDto>,
) -> CommandResult<Vec<RecentVaultDto>> {
    let dirs = current_app_directories()?;
    let vaults: Vec<RecentVault> = vaults.into_iter().map(recent_vault_from_dto).collect();
...
#[tauri::command]
pub(crate) fn backup_vault(vault_path: String, backup_path: String) -> CommandResult<()> {
...
#[tauri::command]
pub(crate) fn export_top_sources(
    output: String,
    metric: String,
    source_type: String,
    limit: usize,
...
#[tauri::command]
pub(crate) fn export_search_sources(
    output: String,
    req: SearchSourcesRequest,
    state: tauri::State<AppState>,
) -> CommandResult<Value> {
